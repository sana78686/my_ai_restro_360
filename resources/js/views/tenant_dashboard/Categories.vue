...
export default {
  ...
  setup() {
    const { t } = useI18n() // Correct: use 't' instead of '$t'
    ...
    const showDeletedModal = async () => {
      try {
        ...
        Swal.fire({ 
          ...
          text: t('tenant_dashboard.categories.messages.deleteFailed') 
        })
      }
    }
      ...
      try {
        ...
        Swal.fire({
          ...
          title: isEditing.value 
            ? t('tenant_dashboard.categories.categoryUpdated') 
            : t('tenant_dashboard.categories.categoryCreated'),
          toast: true,
          position: 'top-end',
    ...
    const confirmDelete = (category) => {
      Swal.fire({
        ...
        cancelButtonText: t('dashboard_common.cancel')
      }).then(async (result) => {
    ...
    const onSortEnd = async () => {
      try {
        ...
        Swal.fire({ 
          icon: 'success', 
          title: t('tenant_dashboard.categories.orderUpdated'), 
          toast: true, 
          position: 'top-end', 
          showConfirmButton: false, 
          timer: 1500 
        ...
        Swal.fire({ 
          icon: 'error', 
          title: t('dashboard_common.error'), 
          text: t('tenant_dashboard.categories.orderUpdateFailed') 
        });
      }
    };
